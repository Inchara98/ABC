...
class Test_PM_Poshan:
    ...
    def test_click_on_implementation_status_tab(self):
        ...
        time.sleep(2)
        result = self.driver.find_element(By.XPATH, self.pageobjects.pm_implementation_tab).get_attribute(
            'aria-selected')
        if "true" == result:
    ...
    def test_click_on_state_check_with_metrics_options(self):
        ...
        for i in range(count-1):
            state_ids = self.driver.find_element(By.XPATH, "//div[starts-with(@id,'a') and contains(@id,"'-'+str(i)+")]")
            state_ids.click()
            time.sleep(4)
            result = self.data.get_map_tooltip_info_validation(self, driver=self.driver)
            print(result)
